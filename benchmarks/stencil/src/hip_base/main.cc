...
int main(int argc, char** argv) {
    ...
    {
        hipLaunchKernel(HIP_KERNEL_NAME(naive_kernel), dim3(grid), dim3(block), 0, 0, c0,c1, d_A0, d_Anext, nx, ny,  nz);
        float *d_temp = d_A0;
        d_A0 = d_Anext;
        d_Anext = d_temp;

    }
    HIPERR // check and clear any existing errorsi
    float *d_temp = d_A0;
    d_A0 = d_Anext;
    d_Anext = d_temp;
