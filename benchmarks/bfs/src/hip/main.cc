...
int main(int argc, char** argv) 
{
  ...
  do
  {
    ...
    if(k%2 == 0){
      if(num_of_blocks == 1){
        hipLaunchKernel(HIP_KERNEL_NAME(BFS_in_GPU_kernel), dim3(grid), dim3(threads ), 0, 0, d_q1,d_q2, d_graph_nodes, 
        ...
        hipLaunchKernel(HIP_KERNEL_NAME(BFS_kernel_multi_blk_inGPU), dim3(grid), dim3(threads ), 0, 0, d_q1,d_q2, d_graph_nodes, 
      ...
      if(num_of_blocks == 1){
        hipLaunchKernel(HIP_KERNEL_NAME(BFS_in_GPU_kernel), dim3(grid), dim3(threads ), 0, 0, d_q2,d_q1, d_graph_nodes, 
            d_graph_edges, d_color, d_cost, num_t, tail,GRAY1,k,d_overflow);
      }
      else if(num_of_blocks <= NUM_SM){
        (hipMemcpy(num_td,&num_t,sizeof(int),
                    hipMemcpyHostToDevice));
        hipLaunchKernel(HIP_KERNEL_NAME(BFS_kernel_multi_blk_inGPU), dim3(grid), dim3(threads ), 0, 0, d_q2,d_q1, d_graph_nodes, 
