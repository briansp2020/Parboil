...
texture<Node, 1, hipReadModeElementType> g_graph_node_ref;
texture<Edge, 1, hipReadModeElementType> g_graph_edge_ref;
...
struct LocalQueues {
  ...
  __device__ void reset(int index) {
    ...
    sharers[index] =
      (hipBlockDim_x >> EXP) +
      (hipThreadIdx_x < (hipBlockDim_x & MOD_OP));
  }
  ...
  __device__ void concatenate(int *dst, int (&prefix_q)[NUM_BIN]) {
    ...
    int q_i = hipThreadIdx_x & MOD_OP; // w-queue index
    int local_shift = hipThreadIdx_x >> EXP; // shift within a w-queue
...
//--------------------------------------------------
__global__ void
BFS_in_GPU_kernel(grid_launch_parm lp,
                  int *q1, 
                  int *q2, 
                  Node *g_graph_nodes, 
                  Edge *g_graph_edges, 
...
//--------------------------------------------------------------
__global__ void
BFS_kernel_multi_blk_inGPU(grid_launch_parm lp,
                           int *q1,
                           int *q2, 
                           Node *g_graph_nodes, 
                           Edge *g_graph_edges, 
           ...
           int *overflow) 
{
  ...
  if( tid < no_of_nodes)
  {
    // Visit a node from the current frontier; update costs, colors, and
    // output queue
    visit_node(q1[tid], hipThreadIdx_x & MOD_OP, g_graph_nodes, g_graph_edges,
               local_q, overflow, g_color, g_cost, gray_shade);
  }
  __syncthreads();
